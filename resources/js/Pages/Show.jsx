  ...
  return (
    ...
    <Head title={posts.title.slice(0,5)}/>
      <div className=' flex justify-center'>
          ...
          <img className="p-5 roubded-lg object-cover w-full rounded-t-lg h-96 md:h-auto  md:rounded-none md:rounded-s-lg"
            ...
            <div className='relative'>
             ...
             <FontAwesomeIcon icon='ellipsis'></FontAwesomeIcon>
                ...
                {/* show model  */}
            {showmodel && (
          <div className={`absolute z-50 ${!canmodify ? 'top-[-100px]':'top-[-250px]'} right-[-6px] border dark:border-slate-200 bg-white dark:bg-slate-800 text-white rounded-lg overflow-hidden w-40`}>
            {canmodify &&(
              <>
              <Link href={route('posts.edit',posts.id)} className='block w-full px-6 py-3 dark:hover:bg-slate-700 hover:bg-gray-200/20 text-gray-800/80 dark:text-white text-left '>Edit</Link>
              <button onClick={() => handleDelete(posts.id)} className='block w-full px-6 py-3 dark:hover:bg-slate-700 hover:bg-gray-200/20 text-gray-800/80 dark:text-white text-left '>Delete</button>
              </>
            )}
        ...
        {canreport && 
        <button onClick={()=>{setshowReportmodel(!showReportmodel)}} className="block w-full px-6 py-3 dark:hover:bg-slate-700 hover:bg-gray-200/20 text-gray-800/80 dark:text-white text-left ">Report</button>
        }
         {userLikeCount > 0 && (
              <button onClick={() => { handleUndo(); setShowModel(false); }} className="block w-full px-6 py-3 dark:hover:bg-slate-700 hover:bg-gray-200/20 text-gray-800/80 dark:text-white text-left">
      ...
      <div className="relative container mx-auto my-10 lg:w-[30%] w-fit h-14 space-x-2 flex justify-center items-center gap-2 border-2 border-gray-200/60 dark:border-0 rounded-full px-2 sm:px-6 py-3 text-md lg:text-xl dark:bg-gray-800/80 bg-white/70">
         ...
         <span onClick={opencomment} className="px-2  h-full flex items-center justify-center gap-1 cursor-pointer">
           <FontAwesomeIcon icon={['far','comment']} className=" dark:text-white mr-1"></FontAwesomeIcon> 
      ...
      {morearticles.length >0 && (
        <>
      <p className="dark:text-white/60 border-b-2 w-fit mx-auto text-center border-b-gray-200/60 text-lg font-semibold mt-5 capitalize">More Articles</p>
